...
const quiries       =    webservises.map(function(webservice){
                              let d = new Date();
                              return    `Select EVENT_SYSTEM,event_call_type,status,count(*) as countOf ` + 
										`from EVENT_LOG where EVENT_SYSTEM = '${webservice}' ` + 
										`and event_start_time >= TO_date('${d.getPySHDateTime(2)}' `+
										`, 'SYYYY-MM-DD HH24:MI:SS') and event_start_time <= TO_date('${d.getPySHDateTime()}', 'SYYYY-MM-DD HH24:MI:SS') ` + 
										`group by EVENT_SYSTEM,event_call_type,status order by status,event_call_type,countOf `
                         });
...
connect
     .then(function(connection){
     	const funcs = quiries.map(q=>()=>executeQueryPromise(connection,q))
		console.log(funcs);
			/*
          const q1 = executeQueryPromise(connection,quiries[0]);
          const q2 = executeQueryPromise(connection,quiries[1]);
          const q3 = executeQueryPromise(connection,quiries[2]);
          const q4 = executeQueryPromise(connection,quiries[3]);
          ...
          .then(outPutResult)*/
        Promise.resolve([])
        	.then(function(allResultArry){
        		return funcs[0]().then(function(dbResult){
        			return allResultArry.push(dbResult)
        		});
        	})
        	.then(outPutResult);
     });
